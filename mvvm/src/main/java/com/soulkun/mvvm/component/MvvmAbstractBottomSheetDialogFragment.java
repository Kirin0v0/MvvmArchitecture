...
/**
 * @author soulkun
 * @time 2022/8/22 17:38
 * @description MVVM框架下的BottomSheetDialogFragment基础类
 */
public abstract class MvvmAbstractBottomSheetDialogFragment extends BottomSheetDialogFragment {
    ...
    public Dialog onCreateDialog(@Nullable final Bundle savedInstanceState) {
        // 生成内存不泄露的BottomSheetDialog
        return new NoLeakBottomSheetDialog(getContext(), getTheme());
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        initWindow(bottomSheetDialog.getWindow());

        final View bottomSheetView = bottomSheetDialog.getDelegate().findViewById(com.google.android.material.R.id.design_bottom_sheet);
        if (bottomSheetView != null) {
            mBottomSheetBehavior = BottomSheetBehavior.from(bottomSheetView);
            initBottomSheetBehavior(mBottomSheetBehavior);
        }
    }
    ...
    protected abstract void initWindow(final @NonNull Window window);

    protected abstract void initBottomSheetBehavior(final @NonNull BottomSheetBehavior<View> bottomSheetBehavior);
     ...
     */
    public static class NoLeakBottomSheetDialog extends BottomSheetDialog {

        public NoLeakBottomSheetDialog(@NonNull final Context context) {
            super(context);
        }

        public NoLeakBottomSheetDialog(@NonNull final Context context, final int theme) {
            super(context, theme);
        }

        public NoLeakBottomSheetDialog(@NonNull final Context context, final boolean cancelable, final DialogInterface.OnCancelListener cancelListener) {
            super(context, cancelable, cancelListener);
        }

        @Override
        public void setOnCancelListener(@Nullable final DialogInterface.OnCancelListener listener) {
            super.setOnCancelListener(new MvvmAbstractDialogFragment.NoLeakDialog.OnCancelListener(listener));
        }

        @Override
        public void setOnDismissListener(@Nullable final DialogInterface.OnDismissListener listener) {
            super.setOnDismissListener(new MvvmAbstractDialogFragment.NoLeakDialog.OnDismissListener(listener));
        }
